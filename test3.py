...
def begin(driver, number):
    ...
    while i <= 1000:
        ...
        if i == 100:
            ...
            try:
                img.screenshot(os.path.join("captcha", f"{threading.currentThread().name}.png"))
            except:
                try:
            ...
            res = ocr.ocr_local(os.path.join("captcha", f"{threading.currentThread().name}.png"))
        captcha.clear()
...
def for_main(q: queue.Queue):
    print("{%s} 进程启动。。。" % threading.Thread.name)
    driver = get_driver.get_driver()
    driver.get("https://www.beckett-authentication.com/verify-certificate")
    while True:
        if q.empty():
            print("{%s} 进程结束了。。。" % threading.currentThread().name)
            break
        number = q.get()
        if os.path.exists(f"data/{number}.json"):
            print(f"{threading.currentThread().name} 进程发现 {number}.json存在了")
        else:
            try:
                begin(driver, number)
...
def multi_process_main():
    ...
    for number in numbers:
        q.put(number.strip())
    threading.Thread(target=for_main, args=(q,)).start()
    threading.Thread(target=for_main, args=(q,)).start()
    threading.Thread(target=for_main, args=(q,)).start()
    threading.Thread(target=for_main, args=(q,)).start()
    threading.Thread(target=for_main, args=(q,)).start()
...
if __name__ == '__main__':
    # main()
    start = time.time()
    p = threading.Thread(target=multi_process_main)
    p.start()
    p.join()
    end = time.time()
    print(end - start)
